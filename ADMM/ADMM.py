...
class ADMM:
	def __init__(self, problem_data, rho_method):
		...
		self.r_prim_norm = self.pd.r_prim_norm
		self.r_dual_norm = self.pd.r_dual_norm
	...
	def stop_criteria(self, k):
		eps_max_prim = np.amax(np.array([np.linalg.norm(csr_matrix.dot(self.Id, self.r[k+1])),np.linalg.norm(csr_matrix.dot(- self.Id, self.p[k+1])), 0))
		eps_pri = np.sqrt(p)*ABSTOL + RELTOL*eps_max_prim

		eps_max_dual = np.linalg.norm(self.rho * csc_matrix.dot(self.Id, xi[k + 1]))
		eps_dual = np.sqrt(self.n) * ABSTOL + RELTOL * eps_max_dual

		self.r_prim_norm.append(np.linalg.norm(self.r_prim[k + 1]))
		self.r_dual_norm.append(np.linalg.norm(self.r_dual[k + 1]))

Este criterio va en el codigo cuando use esta clase

		if self.r_prim_norm[k + 1] <= eps_pri and self.r_dual_norm[k + 1] <= eps_dual:
			#break
